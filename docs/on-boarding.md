...
3. `tools/serve-mona2-web.sh` で自分の moNa2 web tools を起動する
4. Chrome / Edge で `http://127.0.0.1:14242/studio/` を開き、USB で moNa2_R に接続する
5. ZMK Studio で keymap を変更して保存する

ローカルで使う場合は `localhost` で動作します。別端末から使う場合や Tailscale URL で使う場合は、既存の `:443` route を上書きしないよう `14242` などの moNa2 専用ポートで Tailscale Serve してください。

```
./tools/serve-mona2-web.sh
```

Tailscale URL から使う場合は、以下のように同じポートで Studio と flasher を公開します。

```
/Applications/Tailscale.app/Contents/MacOS/Tailscale serve --bg --https=14242 http://127.0.0.1:14242
```

UF2 をブラウザから流し込みたい場合は、同じサーバーのトップページを使います。
...
ブラウザで流し込む場合は、moNa2 web tools を起動します。

```
./tools/serve-mona2-web.sh
```

Chrome / Edge で `http://127.0.0.1:14242/` を開き、UF2 ファイルと `XIAO-SENSE` ドライブを選択してください。Tailscale URL から使う場合は、Tailscale Serve で HTTPS として公開してください。
