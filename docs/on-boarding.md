...
時間があれば、先に右側 central 用の ZMK Studio 対応ファームウェアだけを書き込んでおくと良いでしょう。

#### 大まかな流れ

1. 組立済みの moNa2 が手元に届く
2. キースイッチ と キーキャップ を取り付ける
3. 右側 central に ZMK Studio 対応ファームウェアを一度だけ転送する
4. self-host した ZMK Studio を開く
5. USB 経由で moNa2_R に接続して keymap を変更する
6. 復旧や大きな設定変更が必要な場合だけ、UF2 ファームウェアを転送する

## 1. 推奨フロー

通常の keymap 変更では、他人の Web サイト、GitHub OAuth、main ブランチへの直接 push、毎回の GitHub Actions build は使いません。

推奨フローは以下です。

1. このリポジトリで ZMK Studio 対応の右側 firmware を作る
2. 右側 moNa2_R にその firmware を一度だけ転送する
3. `tools/serve-zmk-studio.sh` で自分の ZMK Studio を起動する
4. Chrome / Edge から USB で moNa2_R に接続する
5. ZMK Studio で keymap を変更して保存する

ローカルで使う場合は `localhost` で動作します。別端末から使う場合や Tailscale URL で使う場合は、既存の `:443` route を上書きしないよう専用ポートで Tailscale Serve してください。

```
./tools/serve-zmk-studio.sh
```

Tailscale URL から ZMK Studio を開く場合は、以下のように専用ポートで公開します。

```
/Applications/Tailscale.app/Contents/MacOS/Tailscale serve --bg --https=5173 http://127.0.0.1:5173
```

UF2 をブラウザから流し込みたい場合は、以下の self-host flasher を使います。
...
- keymap を解釈して Mac / PC に HID 入力を送るのは右側 central です。
- 左側は基本的に、押されたキー位置を右側に送る役割です。
- `config/moNa2.keymap` の通常のキー配置変更だけなら、右側 firmware の更新で反映されます。

左側 firmware も必要になるのは、左側 overlay、matrix、GPIO、encoder、split 通信、左右両方に関係する firmware 設定を変えたときです。復旧時や初期状態が不明なときは、左右両方を揃えて焼くほうが安全です。

## 3. Legacy: GitHub リポジトリのフォーク

以下は従来フローです。通常運用では推奨しません。
...
## 4. Legacy: KeymapEditor との連携

zmk の キーマップ変更は `config/moNa2.keymap` などの `.keymap` ファイルを編集して main ブランチに反映する必要があります。
...
この方法は外部 Web サービスに GitHub OAuth 権限を渡します。通常は self-host ZMK Studio を使ってください。
...
通常の keymap 変更だけなら、右側の `moNa2_R rgbled_adapter-seeeduino_xiao_ble-zmk.uf2` だけを転送します。`settings_reset` は不要です。

復旧や初期化を兼ねる場合は、以下の手順で転送しましょう。
...
ブラウザで流し込む場合は、self-host flasher を起動します。

```
./tools/serve-uf2-flasher.sh
```

Chrome / Edge で `http://localhost:8787` を開き、UF2 ファイルと `XIAO-SENSE` ドライブを選択してください。Tailscale URL から使う場合は、Tailscale Serve で HTTPS として公開してください。

```
/Applications/Tailscale.app/Contents/MacOS/Tailscale serve --bg --https=8787 http://127.0.0.1:8787
```
